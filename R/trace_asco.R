...
#' @param primary_infection_foci refers to the coordinates where the infection
#'   is first observed. Accepted inputs are: `centre`/`center` or `random`
...
#'   Defaults to `FALSE`.
#' @param fungicide_dates a character vector of dates indicating when a preventative
#'   fungicide application is made. Dates need to be supplied in ISO8601 format
#'   (YYYY-MM-DD), _e.g._ \dQuote{2020-04-26}.
#'
...
trace_asco <- function(weather,
  ...
  for (i in seq_len(length(time_increments))) {
    ...
    daily_vals_list[[i + 1]] <- one_day(
      ...
      fungicide_dates = fungicide_dates
    )
