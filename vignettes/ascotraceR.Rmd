...
### Simulate fungicide applications  

The function, `trace_asco()`, also can simulate the effect of preventative fungicides.
`fungicide_dates` protect chick pea growing points prior to the specified date/s
preventing them from being *exposed* and *infective*. 
All growing points maturing after fungicide dates are not protected.

```{r trace-asco-fungicide}
# Predict Ascochyta blight spread for the year 2020 at Billa Billa
fp_trace <- trace_asco(
  weather = Billa_Billa,
  paddock_length = 20,
  ...
  primary_inoculum_intensity = 100,
  primary_infection_foci = "centre",
  fungicide_dates = c("2020-07-25","2020-08-07")
)
```

To show the effect the outputs are summarised and compared in a line plot.

```{r plot-fungicide}
# summarise output
fp_summary <- summarise_trace(fp_trace)

# plot original simulation without fungicide
plot(
  y = summarised$infectious_gp / summarised$susceptible_gp,
  x = summarised$i_date,
  type = "l",
  ylab = "Percent infectious growing points",
  xlab = "Dates"
)
# overlay simulation with fungicide
lines(
  y = fp_summary$infectious_gp / fp_summary$susceptible_gp,
  x = fp_summary$i_date,
  type = "l",
  col = "purple"
)
# add a vertical line to show fungicide application date
abline(v = as.Date(c("2020-07-25","2020-08-07")), col = "blue")
```
